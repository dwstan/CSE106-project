...
class Explore(Resource):
    def get(self, user_id):
        try: 
            ...
            LEFT JOIN 
                (SELECT followed_id FROM follow WHERE follower_id = :profile_id) as user_follow ON post.user_id = user_follow.followed_id
            WHERE
                post.user_id != :profile_id AND user_follow.followed_id IS NULL
            ORDER BY 
                post.date DESC
            """)
